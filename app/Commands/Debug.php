...
trait Debug
{
    ...
    protected function debugReport(): void
    {
        ...
        $this->consoleWriter->text([
            'The following is the configuration Filament Installer has computed by merging:',
        ]);
        ...
        $savedConfig = [];
        if (File::isFile(config('home_dir') . '/.filament/config')) {
            $savedConfig = Dotenv::createMutable(config('home_dir') . '/.filament', 'config')->load();
        }
        $this->arrayToTable(
            $savedConfig,
            [
